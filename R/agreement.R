#' Calculate the quadratically weighted Fleiss' kappa and Conger's (Cohen's) kappa
#'
#' Estimate a quadratically weighted kappa with potentially missing data using least squares, or
#'    calculate its population value. `conger` and `cohen` are aliases.
#'    See [fleissci()] and [cohenci()] for confidence intervals.
#'
...
#'
#' The functions `cohen_pop`, `conger_pop`, and `fleiss_pop` calculate the population values
#'    of the kappas using the mean and covariance of the rating rating distributions.
#'    The derivation of the covariance and mean formulation can be found in (Moss and van Oest, work in progress.)
#'
#' The functions `cohen`, `conger`, and `fleiss` estimate the kappas using least squares.
#'    This method uses the biased sample covariance estimator, which agrees
#'    with the literature on agreement coefficients in the case of two raters.
#'    In the case of missing data, the pairwise available data is used,
#'    employing the option `use = "pairwise.complete.obs"` in [stats::cov()].
#'
#'
#' @param x Input data on wide form. Columns are raters, items are ratings. Missing
#'    values are allowed, and should be encoded as `NA`s.
